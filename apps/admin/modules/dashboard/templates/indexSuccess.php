<br><br><br><br><br><br>
<div align=center>
<h1>Dashboard Center for company <?php echo $companyname ?></h1>
<table border=0>
<tr>
<td>
<?php
...
//echo "<pre>";
 //   print_r($reports);
    ...
    $button_resolved = button_to(' Resolved ', 'dashboard/index?feedbackid='.$reports[$a]['id']);
...
if(count($needworkouts) > 0 ){
  ...
  for($a=0; $a< count($needworkouts); $a++){
    ...
    echo "<tr><td>{$needworkouts[$a]['email']} {$needworkouts[$a]['user_firstname']}</td> <td>".button_to('Show Workout list', "admin/schedulelist?user_id={$needworkouts[$a]['user_id']}")."</td> <td align=center>$lastsetworkout</td></tr>";
  }
